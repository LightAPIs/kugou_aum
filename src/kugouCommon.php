...
class AumKugouCommon {
    ...
    public static function processLrcLine($lrc) {
        ...
        foreach (explode("\n", $lrc) as $line) {
            ...
            if (!AumKugouCommon::isValidLrcTime($key) || !AumKugouCommon::isValidLrcText($value)) {
                $key = '';
                $value = $line;
            }
    ...
    public static function getChineseTranslationLrc($orgLrc, $transLrc) {
        $resultLrc = '';
        $orgLines = AumKugouCommon::processLrcLine($orgLrc);
        $transLines = AumKugouCommon::processLrcLine($transLrc);
        ...
        foreach ($orgLines as $line) {
            ...
            $trans = '';
            if ($key !== '') {
                $time = AumKugouCommon::getTimeFromTag($key);
                for ($i = $transCursor; $i < count($transLines); $i++) {
                    $tKey = $transLines[$i]['tag'];
                    if (AumKugouCommon::getTimeFromTag($tKey) > $time) {
                        $transCursor = $i;
                        break;
                    }
